...
def _submit_identity_order(env: MiniBGEnv) -> None:
    if env.legal_actions_mask[A_FINISH]:
        env.step(A_FINISH)
    if env._state.players[env._state.current_player_index].phase == PlayerPhase.ORDER:
        env.step(A_FINISH)
...
def test_order_phase_legal_swap_count():
    """ORDER phase: FINISH plus adjacent swaps among occupied positions (k−1 when k≥2)."""
    env = MiniBGEnv(seed=0)
    for k in range(BOARD_SIZE + 1):
        ...
        n_legal = int(m[A_FINISH]) + sum(
            int(m[A_SWAP_BOARD_0 + i]) for i in range(NUM_SWAP_ADJ)
        )
        assert n_legal == max(1, k), f"k={k}"
...
def test_swap_action_in_shop_phase_is_illegal():
    env = MiniBGEnv(seed=0)
    snapshot = env.get_state_hash()
    res = env.step(A_SWAP_BOARD_0)
    assert res.info["invalid_action"] is True
