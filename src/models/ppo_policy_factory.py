...
PPO_NETWORK_BGLIKE_STRUCTURED_V8 = "bglike_structured_v8"
...
PPO_NETWORK_BGLIKE_STRUCTURED_V13 = "bglike_structured_v13"
...
def build_ppo_actor_critic(
    ...
    *,
    ...
    if nt == PPO_NETWORK_BGLIKE_STRUCTURED_V8:
        from src.envs.bglike.obs_v5 import OBS_DIM_V5 as _BG_OBS_DIM_V5
        ...
        return BGLikeStructuredV8(
            slot_hidden=int(slot_hidden_channels),
            region_conv2_kernel=int(region_conv2_kernel),
            card_emb_dim=int(card_emb_dim),
            num_pool_indices=num_pool_indices,
        )
    if nt == PPO_NETWORK_BGLIKE_STRUCTURED_V9:
        from src.envs.bglike.obs_v5 import OBS_DIM_V5 as _BG_OBS_DIM_V5
        ...
        return BGLikeStructuredV9(
            slot_hidden=int(slot_hidden_channels),
            region_conv2_kernel=int(region_conv2_kernel),
            card_emb_dim=int(card_emb_dim),
            num_pool_indices=num_pool_indices,
        )
    if nt == PPO_NETWORK_BGLIKE_STRUCTURED_V10:
        from src.envs.bglike.obs_v5 import OBS_DIM_V5 as _BG_OBS_DIM_V5
        from .bglike_structured_v10 import BGLikeStructuredV10

        if len(observation_shape) != 1 or int(observation_shape[0]) != _BG_OBS_DIM_V5:
            raise ValueError(
                f"network_type {nt!r} requires observation_shape [{_BG_OBS_DIM_V5}]"
            )
        return BGLikeStructuredV10(
            slot_hidden=int(slot_hidden_channels),
            region_conv2_kernel=int(region_conv2_kernel),
            card_emb_dim=int(card_emb_dim),
            num_pool_indices=num_pool_indices,
        )
    if nt == PPO_NETWORK_BGLIKE_STRUCTURED_V11:
        from src.envs.bglike.obs_v5 import OBS_DIM_V5 as _BG_OBS_DIM_V5
        from .bglike_structured_v11 import BGLikeStructuredV11
    ...
    if nt == PPO_NETWORK_BGLIKE_STRUCTURED_V13:
        from src.envs.bglike.obs_v7_pref import OBS_DIM_V7_PREF as _BG_OBS_DIM_V7
        from .bglike_structured_v13 import BGLikeStructuredV13

        if len(observation_shape) != 1 or int(observation_shape[0]) != _BG_OBS_DIM_V7:
            raise ValueError(
                f"network_type {nt!r} requires observation_shape [{_BG_OBS_DIM_V7}]"
            )
...
def _restore_structured_v13(
    obs_shape: Tuple[int, ...], num_actions: int, kw: Dict[str, Any]
) -> nn.Module:
    from .bglike_structured_v13 import BGLikeStructuredV13

    if not kw.get("num_pool_indices"):
        raise ValueError("num_pool_indices is required to restore structured_v13 checkpoint")
    return BGLikeStructuredV13(**kw)


def _register_v13_lazy() -> None:
    from .bglike_structured_v13 import BGLikeStructuredV13

    register_ppo_network(
        PPO_NETWORK_BGLIKE_STRUCTURED_V13,
        BGLikeStructuredV13,
        restore=_restore_structured_v13,
    )
...
def _restore_structured_v8(
    obs_shape: Tuple[int, ...], num_actions: int, kw: Dict[str, Any]
) -> nn.Module:
    from .bglike_structured_v8 import BGLikeStructuredV8

    if not kw.get("num_pool_indices"):
        raise ValueError("num_pool_indices is required to restore structured_v8 checkpoint")
    return BGLikeStructuredV8(**kw)


def _register_v8_lazy() -> None:
    from .bglike_structured_v8 import BGLikeStructuredV8

    register_ppo_network(
        PPO_NETWORK_BGLIKE_STRUCTURED_V8,
        BGLikeStructuredV8,
        restore=_restore_structured_v8,
    )


def _restore_structured_v9(
    obs_shape: Tuple[int, ...], num_actions: int, kw: Dict[str, Any]
) -> nn.Module:
    from .bglike_structured_v9 import BGLikeStructuredV9

    if not kw.get("num_pool_indices"):
        raise ValueError("num_pool_indices is required to restore structured_v9 checkpoint")
    return BGLikeStructuredV9(**kw)


def _register_v9_lazy() -> None:
    from .bglike_structured_v9 import BGLikeStructuredV9

    register_ppo_network(
        PPO_NETWORK_BGLIKE_STRUCTURED_V9,
        BGLikeStructuredV9,
        restore=_restore_structured_v9,
    )


def _restore_structured_v10(
    obs_shape: Tuple[int, ...], num_actions: int, kw: Dict[str, Any]
) -> nn.Module:
    from .bglike_structured_v10 import BGLikeStructuredV10

    if not kw.get("num_pool_indices"):
        raise ValueError("num_pool_indices is required to restore structured_v10 checkpoint")
    return BGLikeStructuredV10(**kw)


def _register_v10_lazy() -> None:
    from .bglike_structured_v10 import BGLikeStructuredV10

    register_ppo_network(
        PPO_NETWORK_BGLIKE_STRUCTURED_V10,
        BGLikeStructuredV10,
        restore=_restore_structured_v10,
    )


def _restore_structured_v11(
    obs_shape: Tuple[int, ...], num_actions: int, kw: Dict[str, Any]
) -> nn.Module:
    from .bglike_structured_v11 import BGLikeStructuredV11

    if not kw.get("num_pool_indices"):
        raise ValueError("num_pool_indices is required to restore structured_v11 checkpoint")
    return BGLikeStructuredV11(**kw)


def _register_v11_lazy() -> None:
    from .bglike_structured_v11 import BGLikeStructuredV11

    register_ppo_network(
        PPO_NETWORK_BGLIKE_STRUCTURED_V11,
        BGLikeStructuredV11,
        restore=_restore_structured_v11,
    )
